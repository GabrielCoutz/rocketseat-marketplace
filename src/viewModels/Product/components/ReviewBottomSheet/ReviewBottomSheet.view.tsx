    ...
    <View className="rounded-t-2xl bg-background">
      ...
      {isLoading ? (
        <View className="min-h-[300px] items-center justify-center p-6">
          ...
          <Text className="mt-4 text-center text-gray-600">Verificando avaliação existente...</Text>
        </View>
      ) : (
        <View className="p-6">
          <Text className="text-base font-semibold text-gray-300">Nota</Text>
          <View className="mb-6 flex-row items-center gap-2">
            <Stars rating={ratingForm.rating} onChangeRating={handleRatingChange} />
          </View>

          <AppInput
            onChangeText={handleContentChange}
            label="COMENTÁRIO"
            placeholder={ratingForm.isEditing ? 'Edite sua avaliação' : 'Escreva sua avaliação'}
